...
export async function GET(req: NextRequest) {
    try {
        ...
        let query = supabase.rpc('rpc_get_currencies');
        // Note: rpc_get_currencies already filters by ACTIVE, but we'll apply JS filter if needed
        
        let { data, error } = await query;
        if (error) throw error;
        
        if (searchParams.has('status') && data) {
